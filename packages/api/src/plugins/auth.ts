...
import { UnauthorizedError, ForbiddenError, TokenExpiredError } from '@oldworldcharm/shared';
...
async function authPlugin(fastify: FastifyInstance) {
  ...
  fastify.decorate('authenticate', async function(request: FastifyRequest, reply: FastifyReply) {
    try {
      ...
      reply.code(error instanceof UnauthorizedError || error instanceof TokenExpiredError ? 401 : 500);
      throw error;
    }
  });
  ...
  fastify.decorate('setAuthCookie', function(reply: FastifyReply, token: string) {
